...
export class AmpAnalytics extends AMP.BaseElement {
  ...
  handleRequestForEvent_(request, trigger, event) {
    ...
    let requestPromise = Promise.resolve(request);
      ...
      for (const k in params) {
        if (typeof params[k] == 'string') {
          requestPromise = requestPromise
              .then(() => this.variableService_.expandTemplate(
                  params[k], trigger, this.config_, event)
  ...
  isSampledIn_(trigger) {
    ...
    if (threshold >= 0 && threshold <= 100) {
      const keyPromise = this.variableService_.expandTemplate(
          spec['sampleOn'], trigger, this.config_)
        .then(key => urlReplacementsForDoc(this.win.document).expandAsync(key));
